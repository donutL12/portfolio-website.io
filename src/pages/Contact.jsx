            ...
            <div className="hidden md:flex items-center space-x-1">
              {navItems.map((item) => (
                <a 
                  key={item.name}
                  href={item.href} 
                  className={`relative px-4 py-2 text-slate-300 hover:text-white transition-colors duration-200 font-medium group ${item.href === '/contact' ? 'text-white' : ''}`}
                >
                  <span className="relative z-10">{item.name}</span>
                  <div className={`absolute inset-0 bg-gradient-to-r from-blue-600/0 to-purple-600/0 group-hover:from-blue-600/20 group-hover:to-purple-600/20 rounded-lg transition-all duration-300 ${item.href === '/contact' ? 'opacity-100 from-blue-600/20 to-purple-600/20' : 'opacity-0 group-hover:opacity-100'}`}></div>
                </a>
              ))}
              <a 
                href="/resume" 
